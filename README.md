...
2. The remaining 0.5% of the funds are distributed to public addresses in **{n<sub>bet</sub>}** as reward for having published a consensus data stream. *TODO*: It's needs to be clarified how **{n<sub>bet</sub>}** would be populated by the mastercoin protocol so that the transactions in #1 and #2 would proceed *automagically*).
