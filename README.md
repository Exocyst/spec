...
1. To participate as a data stream node for a given period, **P<sub>reg</sub>**, the node operator is required to make an escrow deposit of **X** MSC to a m-of-{n} multisig output where: **m<sub>reg</sub>** is equal to the number of decentralized nodes required to publish a node violation claim, **{n<sub>reg</sub>}** is the public addresses of all nodes currently registered to publish data streams for this market for the entirety of **P<sub>reg</sub>**, and **X = C • P<sub>reg</sub>**, where **C** is the prescribed cost multiplier and **P<sub>reg</sub>** is period the node is registering for.
2. Registered nodes are required to publish data from a prescribed source to the MasterCoin protocol for the registered period, **P<sub>reg</sub>**.
3. Each data stream publishing node verifies the content of other nodes for potential violation claims. If a violation claim is signed by **m<sub>reg</sub>** number of nodes, then the violator's escrowed deposit is distributed amongst the **m<sub>reg</sub>** reporting nodes. If the registered period, **P<sub>reg</sub>**, expires with no violation claims, then the node receives its escrow deposit back. (*TODO*: there needs to be a connection between the violation claim and the txid of the data in question. Perhaps each node claiming violation would send metadata containing the txid of the violating data & the txid of its consensus data to violator's multisig escrow address, as well as the MP data and Exodus addresses.)

####The special case of the claiming transaction:
1. The MasterCoin Protocol understands that this decentralized bet will be arbitrated by an m-of-{n} multisig address  where: **m<sub>bet</sub>** is equal to the number of decentralized nodes required to sign a claiming transaction and **{n<sub>bet</sub>}** is the public addresses of all registered nodes publishing consensus data streams during the length of the bet. Therefore, at the completion of the betting period, **P<sub>reg</sub>**, 99.5% of the proceeds of the wager will *automagically* exist in a multisig m-of-{n} output belonging to the aforementioned arbiter nodes. *TODO*: Seek clarity on whether MasterCoin Protocol-issued currency can be moved to a multisig m-of-{n} output without an actual transaction taking place on the blockchain. It seems the funds in the centralized data stream spec are moving automagically to the bet winner upon completion.
2. The remaining 0.5% of the funds are distributed to the public addresses in **{n<sub>bet</sub>}** as reward for having published consensus data streams.
3. The claimant initiates the claiming transaction by publishing a transaction of the coins held in the **m<sub>bet</sub>-of-{n<sub>bet</sub>}** multisig output to their MasterCoin address.
4. If a given node from **{n<sub>bet</sub>}**, agrees with the conditions of the claimant, then it is responsible for signing the claimant's transaction from the multisig output to the claimant's address. (*NOTE*: there is a rather substantial possibility that I don't properly understand multisig transactions and this whole idea of using multisig m-of-n transactions to distribute arbitration responsibility is bollocks.)
