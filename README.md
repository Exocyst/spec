...
3. Each data stream publishing node verifies the content of other nodes for potential violation claims. If a violation claim is signed by **m<sub>reg</sub>** number of nodes, then the node deposit is distributed amongst the **m<sub>reg</sub>** reporting nodes. If the registered period, **P<sub>reg</sub>**, expires with no violation claims, then the node receives its escrow deposit back. (*TODO*: there needs to be connection between the violation claim and the txid of the data in question. Perhaps each node claiming violation would send metadata containing the txid of the violating data & the txid of its consensus data to violator's multisig escrow address, as well as the MP data and Exodus addresses.)

####The special case of the claiming transaction:
1. The MasterCoin Protocol understands that this decentralized bet will be arbitrated by a m-of-{n} multisig address  where: **m<sub>bet</sub>** is equal to the number of decentralized nodes required sign a claiming transaction and **{n<sub>bet</sub>}** is the public addresses of all registered nodes publishing consensus data streams during the length of the bet. Therefore, at the completion of the betting period, **P<sub>reg</sub>**, 99.5% of the proceeds of the wager will *automagically* exist in the aforementioned multisig m-of-{n} output. *TODO*: Seek clarity on whether mastercoin protocol issued currency can be moved to a multisig m-of-{n} output without an actual transaction taking place on the blockchain. It seems the funds in the central data stream spec are moving automagically the bet winner upon completion.
